...
TB_Function* test_fib(TB_Module* m) {
    ...
    // if (n < 2) return n
    {
        tb_inst_if(func, tb_inst_cmp_ilt(func, n, tb_inst_sint(func, TB_TYPE_I32, 2), true),
            if_true, if_false);
    ...
    // else return fib(n + 1) + fib(n + 2)
    {
        ...
        TB_Register n_minus_one =
            tb_inst_sub(func, n, tb_inst_sint(func, TB_TYPE_I32, 1), TB_ASSUME_NUW);

        TB_Register call1 =
            tb_inst_call(func, TB_TYPE_I32, func, 1, (TB_Register[]) { n_minus_one });

        TB_Register n_minus_two =
            tb_inst_sub(func, n, tb_inst_sint(func, TB_TYPE_I32, 2), TB_ASSUME_NUW);

        TB_Register call2 =
            tb_inst_call(func, TB_TYPE_I32, func, 1, (TB_Register[]) { n_minus_two });
