...
static TreeNodeIndex gen_tree(Ctx* restrict ctx, TB_Function* f, TB_Register r) {
	...
	// TODO(NeGate): Clean this up!!!
	bool share = ctx->should_share[r / 8] & (1u << (r % 8));
	
	if (TB_UNLIKELY(share || r < ctx->last_fence || r >= ctx->root_reg)) {
		...
		if (reg_type != TB_UNSIGNED_CONST && 
			...
			reg_type != TB_LOCAL && 
			reg_type != TB_PHI2) {
			StackSlot* slot = try_get_stack_slot(ctx, f, r, f->nodes.dt[r]);
			if (slot) return push_leaf(ctx, r);
		}
	}
...
static Val alloc_xmm(Ctx* restrict ctx, TB_Function* f, TB_DataType dt) {
	if (ctx->xmm_allocator == 0xFFFF) {
		...
		loop_range(i, 1, ctx->tree_len) if (ctx->tree[i].val.type == VAL_XMM) {
			printf("%s: expr XMM%d spill\n", f->name, ctx->tree[i].val.xmm);
			
			TB_DataType dt = ctx->tree[i].val.dt;
...
#define kill(n) (ctx->tree[n].val = (Val){ 0 })
...
static Val eval(Ctx* restrict ctx, TB_Function* f, TB_Register root_reg, TB_Register next) {
	...
	loop_range(n, 1, ctx->tree_len) {
		// Try to spill if things are filling up
		for (size_t steps = 0; tb_popcount(ctx->gpr_allocator) >= 12 && steps < 2; steps++) {
			loop_range(j, 1, n) {
				if (ctx->tree[j].val.type == VAL_GPR) {
		...
		if (r != root_reg) {
			ctx->use_count[r] -= 1;
			//printf("%s:r%u: use: r%u (%d uses left)\n", f->name, root_reg, r, ctx->use_count[r]);
		}
		...
		switch (reg_type) {
			...
			case TB_ARRAY_ACCESS: {
				Val base = val_addressof(ctx, f, use(node->a));
				Val index = val_rvalue(ctx, f, use(node->b), ctx->tree[node->b].reg);
				...
				if (index.type == VAL_IMM) {
					...
					if (!recycle) free_val(ctx, f, index);
					free_val(ctx, f, base);
				}
...
#else
					// Maybe we've expended all uses of a specific register so we
					// can recycle even if it's owned.
					if (ctx->use_count[ctx->tree[node->a].reg] == 0) {
						//printf("%s: Special recycle with r%u -> r%u\n", f->name, ctx->tree[node->a].reg, r);
						recycle = true;
						val = a;
						val.is_owned = true;
					} else {
