...
static void   add_phi_operand(
      Mem2Reg_Ctx* restrict c, TB_Function* f, TB_Reg phi_reg, TB_Label label, TB_Reg reg);
static TB_Reg try_remove_trivial_phi(Mem2Reg_Ctx* restrict c, TB_Function* f, TB_Reg phi_reg);
static TB_Reg add_phi_operands(
    Mem2Reg_Ctx* restrict c, TB_Function* f, TB_Reg phi_reg, TB_Label label, int var);
...
static TB_Reg add_phi_operands(
    Mem2Reg_Ctx* restrict c, TB_Function* f, TB_Reg phi_reg, TB_Label block, int var) {
...
static TB_Reg try_remove_trivial_phi(Mem2Reg_Ctx* restrict c, TB_Function* f, TB_Reg phi_reg) {
    ...
    loop(i, use_count) if (uses[i] == phi_reg) { try_remove_trivial_phi(c, f, uses[i]); }
...
bool tb_opt_mem2reg(TB_Function* f) {
    ...
    loop(j, c.label_count) { seal_block(&c, j); }
...
#undef X
    ...
    for (TB_Node* n = &f->nodes.data[address]; n != &f->nodes.data[0];
        ...
        if ((n->type == TB_LOAD || n->type == TB_STORE) && n->load.address == address) {
            ...
            if (n->load.is_volatile) {
                ...
                else if (!TB_DATA_TYPE_EQUALS(dt, n->dt))
