...
static TB_Reg cast_into(TB_Function* f, TB_Reg in, TB_DataType to) {
    ...
    else if (dt.type > to.type)
        return tb_inst_trunc(f, in, to);
    ...
    while (n < TRIAL_COUNT) {
        ...
        for (int i = 0; i < inst_count; i++) {
            ...
            if (rng == 0) {
                ...
                pool[pool_size++] = tb_inst_load(
                    f, dt, var_pool[gen_random(0, var_pool_size)], dt.type == TB_I32 ? 4 : 8);
...
int main(int argc, char** argv) {
...

    {
        ...
        for (size_t i = 0; i < thread_count; i++) {
            info[i]           = (FuzzerInfo) { 0 };
            info[i].thread_id = i;
            info[i].initial_state =
                (((uint64_t)gen_random_any()) << 32ull) | ((uint64_t)gen_random_any());
            info[i].initial_inc =
                (((uint64_t)gen_random_any()) << 32ull) | ((uint64_t)gen_random_any());
        ...
        for (size_t i = 0; i < thread_count; i++) {
            threads[i] =
                CreateThread(NULL, 2 << 21, (LPTHREAD_START_ROUTINE)ir_gen, &info[i], 0, 0);
        }
...
static uint32_t gen_random_bool() { return gen_random_any() & 1; }

static TB_DataType gen_random_dt() {
    return (TB_DataType) { .type = (gen_random_any() % TB_MAX_TYPES), .width = 0 };
}

static TB_DataType gen_random_int_dt() {
    return (TB_DataType) { .type = TB_I8 + (gen_random_any() % 4), .width = 0 };
}
