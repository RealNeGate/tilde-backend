...
static _Alignas(64) uint8_t identifier_char_tbl[] = { 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0xff,
    0x03, 0xfe, 0xff, 0xff, 0x87, 0xfe, 0xff, 0xff, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

__attribute__((always_inline)) static bool is_identifier_not_first_char(char ch) {
    size_t i     = ch;
    size_t index = i / 8;
    size_t shift = i & 7;
...
static _Alignas(64) uint8_t space_char_tbl[] = { 0x00, 0x26, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

__attribute__((always_inline)) static bool is_space(char ch) {
    size_t i     = ch;
    size_t index = i / 8;
    size_t shift = i & 7;
...
static bool match(Lexer* restrict l, const char* t) {
    ...
    return str_len == token_len && memcmp(l->token_start, t, token_len) == 0;
}
...
int main(int argc, char** argv) {
    ...
    TB_Module*    m =
        tb_module_create(TB_ARCH_X86_64, TB_SYSTEM_WINDOWS, &features, TB_OPT_O0, 1, true);
                    ...
                    if (match(&l, "ret")) {
                        ...
                        if (l.token_type == TOKEN_NUMBER) {
                            ...
                            if (op == '+') dst = tb_inst_add(current_fn, dt, a, b, TB_ASSUME_NUW);
                                ...
                                dst = tb_inst_sub(current_fn, dt, a, b, TB_ASSUME_NUW);
                            else if (op == '*')
                                dst = tb_inst_mul(current_fn, dt, a, b, TB_ASSUME_NUW);
                            else if (op == '/')
                                dst = tb_inst_div(current_fn, dt, a, b, false);
                            else
                                abort();
