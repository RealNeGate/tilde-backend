...
TB_API bool tb_node_is_terminator(TB_Function* f, TB_Reg r) {
    return f->nodes.data[r].type == TB_IF || f->nodes.data[r].type == TB_GOTO ||
...
static TB_Reg tb_make_reg(TB_Function* f, int type, TB_DataType dt) {
    ...
    f->nodes.data[r] = (TB_Node) { .type = type, .dt = dt, .next = 0 };
...
static TB_Reg tb_bin_arith(
    TB_Function* f, int type, TB_ArithmaticBehavior arith_behavior, TB_Reg a, TB_Reg b) {
...
TB_API void tb_inst_set_scope(TB_Function* f, TB_AttributeID scope) { f->active_attrib = scope; }

TB_API TB_AttributeID tb_inst_get_scope(TB_Function* f) { return f->active_attrib; }
...
TB_API void tb_inst_debugbreak(TB_Function* f) { tb_make_reg(f, TB_DEBUGBREAK, TB_TYPE_VOID); }
...
TB_API TB_Reg tb_inst_load(TB_Function* f, TB_DataType dt, TB_Reg addr, TB_CharUnits alignment) {
    ...
    f->nodes.data[r].load = (struct TB_NodeLoad) { .address = addr, .alignment = alignment };
    return r;
}

TB_API void tb_inst_store(
    TB_Function* f, TB_DataType dt, TB_Reg addr, TB_Reg val, uint32_t alignment) {
    tb_assume(addr);
    tb_assume(val);

    TB_Reg r = tb_make_reg(f, TB_STORE, dt);
    f->nodes.data[r].store =
        (struct TB_NodeStore) { .address = addr, .value = val, .alignment = alignment };
    return;
}

TB_API TB_Reg tb_inst_volatile_load(
    TB_Function* f, TB_DataType dt, TB_Reg addr, TB_CharUnits alignment) {
    tb_assume(f->current_label);

    TB_Reg r = tb_make_reg(f, TB_LOAD, dt);
    f->nodes.data[r].store =
        (struct TB_NodeStore) { .address = addr, .alignment = alignment, .is_volatile = true };
    return r;
}

TB_API void tb_inst_volatile_store(
    TB_Function* f, TB_DataType dt, TB_Reg addr, TB_Reg val, TB_CharUnits alignment) {
    TB_Reg r               = tb_make_reg(f, TB_STORE, dt);
...
TB_API TB_Reg tb_inst_string(TB_Function* f, size_t len, const char* str) {
    ...
    TB_Reg r                = tb_make_reg(f, TB_STRING_CONST, TB_TYPE_PTR);
    f->nodes.data[r].string = (struct TB_NodeString) { .length = len, .data = newstr };
    return r;
}
...
TB_API TB_Reg tb_inst_call(TB_Function* f, TB_DataType dt, const TB_Function* target,
...
TB_API TB_Reg tb_inst_vcall(
    TB_Function* f, TB_DataType dt, const TB_Reg target, size_t param_count, const TB_Reg* params) {
    int param_start = f->vla.count;
...
TB_API void tb_inst_memset(
    TB_Function* f, TB_Reg dst, TB_Reg val, TB_Reg size, TB_CharUnits align) {
    TB_Reg r                = tb_make_reg(f, TB_MEMSET, TB_TYPE_PTR);
    f->nodes.data[r].mem_op = (struct TB_NodeMemoryOp) { dst, val, size, align };
}

TB_API void tb_inst_memcpy(
    TB_Function* f, TB_Reg dst, TB_Reg src, TB_Reg size, TB_CharUnits align) {
