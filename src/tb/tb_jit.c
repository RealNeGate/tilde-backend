...
#error "Cannot compile JIT for this target architecture!"
    ...
    m->compiled_function_pos = malloc(m->functions.count * sizeof(void*));
    ...
    for (size_t i = 0; i < m->functions.count; i++) {
        ...
        size_t prologue =
            codegen->get_prologue_length(m->functions.data[i].output->prologue_epilogue_metadata,
                m->functions.data[i].output->stack_usage);

        size_t epilogue =
            codegen->get_epilogue_length(m->functions.data[i].output->prologue_epilogue_metadata,
                m->functions.data[i].output->stack_usage);
    ...
    loop(i, m->max_threads) {
        loop(j, arrlen(m->ecall_patches[i])) {
            ...
            const TB_External* restrict e =
                &m->externals[p->target_id / per_thread_stride][p->target_id % per_thread_stride];
