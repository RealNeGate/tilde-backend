...
static void schedule_phis(TreeNodeArena* arena, TB_Function* f, TB_Reg* use_count, TB_Reg label_reg,
...
TreeNode* tb_tree_generate(
    TreeNodeArena* arena, TB_Function* f, TB_Reg* use_count, TB_Reg bb, TB_Reg bb_end) {
    if (!arena->base) {
    ...
    if (bb != bb_end) {
        ...
        TB_FOR_EACH_NODE_RANGE(n, f, bb, bb_end) {
            ...
            case TB_STORE: {
                TreeNode* tree_node =
                    push_binary(arena, r, walk(arena, f, use_count, n->store.address),
