...
static const char* compile_bf(
    TB_Function* func, const char* at, TB_Register cells, TB_Register ptr) {
    while (*at) {
        char c = *at++;
        switch (c) {
        case '+': {
...
int main(int argc, char** argv) {
    ...
    TB_Module*    m =
        tb_module_create(TB_ARCH_X86_64, TB_SYSTEM_WINDOWS, &features, TB_OPT_O1, 1, false);
    ...
    TB_Function* func;
    {
        ...
        // clear out the cells
        tb_inst_memset(func, cells, tb_inst_iconst(func, TB_TYPE_I32(1), 0),
            tb_inst_iconst(func, TB_TYPE_I32(1), 512), 4);
