...
static void x64_inst_op64_rr(TB_Emitter* out, uint8_t opcode, uint8_t dst, uint8_t src) {
	tb_out_reserve(out, 3);

	tb_out1b_UNSAFE(out, x64_inst_rex(true, src, dst, 0));
	tb_out1b_UNSAFE(out, opcode | 1);
	tb_out1b_UNSAFE(out, x64_inst_mod_rx_rm(X64_Direct, src, dst));
}
...
static void x64_emit_lea_base_index(TB_Emitter* out, uint8_t dst, uint8_t base, uint8_t index) {
	tb_out_reserve(out, 4);

	tb_out1b_UNSAFE(out, x64_inst_rex(true, dst, base, index));
	tb_out1b_UNSAFE(out, 0x8D);
	tb_out1b_UNSAFE(out, x64_inst_mod_rx_rm(X64_Indirect, dst, X64_RSP));
	tb_out1b_UNSAFE(out, x64_inst_mod_rx_rm(0, (base & 7) == X64_RSP ? X64_RSP : index, base));
}

static void x64_emit_lea_base_disp(TB_Emitter* out, uint8_t dst, uint8_t base, int32_t disp) {
	tb_out_reserve(out, 7);

	tb_out1b_UNSAFE(out, x64_inst_rex(true, dst, base, 0));
	tb_out1b_UNSAFE(out, 0x8D);
	if (disp == (int8_t)disp) {
		tb_out1b_UNSAFE(out, x64_inst_mod_rx_rm(X64_IndirectDisp8, dst, base));
		tb_out1b_UNSAFE(out, disp);
	}
	else {
		tb_out1b_UNSAFE(out, x64_inst_mod_rx_rm(X64_IndirectDisp32, dst, base));

		tb_out1b_UNSAFE(out, disp & 0xFF);
		tb_out1b_UNSAFE(out, (disp >> 8) & 0xFF);
		tb_out1b_UNSAFE(out, (disp >> 16) & 0xFF);
		tb_out1b_UNSAFE(out, (disp >> 24) & 0xFF);
	}
}
...
static void x64_inst_imul_rr(TB_Emitter* out, uint8_t dst, uint8_t src, bool is_64bit) {
	tb_out_reserve(out, 1);

	if (src >= 8 || is_64bit) tb_out1b_UNSAFE(out, x64_inst_rex(is_64bit, src, dst, 0));
	tb_out1b_UNSAFE(out, 0x0F);
	tb_out1b_UNSAFE(out, 0xAF);
	tb_out1b_UNSAFE(out, x64_inst_mod_rx_rm(X64_Direct, src, dst));
}

static void x64_inst_imul_ri32(TB_Emitter* out, uint8_t dst, int32_t imm, bool is_64bit) {
	tb_out_reserve(out, 7);

	if (dst >= 8 || is_64bit) tb_out1b_UNSAFE(out, x64_inst_rex(is_64bit, 0, dst, 0));
	tb_out1b_UNSAFE(out, 0x69);
	tb_out1b_UNSAFE(out, x64_inst_mod_rx_rm(X64_Direct, 0, dst));
...
static bool x64_can_implicit_convert_from(int src_type, int dst_type) {
	if (src_type == dst_type) return true;
	if (src_type == TB_I64 && dst_type == TB_PTR) return true;
	if (src_type == TB_PTR && dst_type == TB_I64) return true;

	// both are ints and the dst is bigger
	if (TB_IS_INTEGER_TYPE(src_type) && TB_IS_INTEGER_TYPE(dst_type) && dst_type <= src_type) return true;

	// both are floats and the dst is bigger
	if (TB_IS_FLOAT_TYPE(src_type) && TB_IS_FLOAT_TYPE(dst_type) && dst_type <= src_type) return true;

	// both are pointers
	if (TB_IS_POINTER_TYPE(src_type) && TB_IS_POINTER_TYPE(dst_type)) return true;
...
static TB_FunctionOutput x64_compile_function(TB_Function* f, const TB_FeatureSet* features) {
	...
	// Do a bit of analysis for the allocations, we wanna minimize dynamic allocation
