...
static Val fast_eval(X64_FastCtx* ctx, TB_Function* f, TB_Reg r) {
    ...
    if (ctx->addresses[r].type) {
        ...
        case ADDRESS_DESC_FLAGS: return val_flags(ctx->addresses[r].flags);
...
// OP lhs, eval(rhs)
static void fast_folded_op_sse(
    X64_FastCtx* ctx, TB_Function* f, Inst2FPType op, const Val* lhs, TB_Reg rhs_reg) {
    Val rhs = fast_eval(ctx, f, rhs_reg);
    ...
    TB_FOR_EACH_NODE_RANGE(n, f, bb, bb_end) {
                ...
                *ctx->header.out++ = ((dst_xmm & 7) << 3) | RBP;
                ...
                if (dt.type == TB_F64) {
                    ...
                    *rdata_payload          = imm;
                    ...
                    disp = tb_emit_const_patch(f->module, f, GET_CODE_POS(), rdata_payload,
        ...
        case TB_STRING_CONST: {
            ...
            *ctx->header.out++ = mod_rx_rm(MOD_INDIRECT, dst_gpr, RBP);
            ...
            uint32_t disp =
                tb_emit_const_patch(f->module, f, GET_CODE_POS(), str, len, s_local_thread_id);
        ...
        case TB_INITIALIZE: {
            ...
            TB_Initializer*  i =
                (TB_Initializer*)&m
                    ->initializers[n->init.id / per_thread_stride][n->init.id % per_thread_stride];
...
TB_FunctionOutput x64_fast_compile_function(TB_FunctionID id, TB_Function* restrict f,
