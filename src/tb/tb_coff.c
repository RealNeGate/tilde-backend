...
void tb_export_coff(TB_Module* m, const ICodeGen* restrict code_gen, const char* path, bool emit_debug_info) {
        ...
        // Write symbol info table
        {
            ...
            if (true) {
                loop(func_id, m->functions.count) {
                    ...
                    uint32_t function_end = (func_id + 1) < m->functions.count ? func_layout[func_id + 1] : text_section.raw_data_size;
    ...
    if (emit_unwind_info) {
        ...
        if (xdata_function_info == NULL) {
            xdata_function_info = tb_platform_heap_alloc(m->functions.count * sizeof(uint32_t));
    ...
    data_section.pointer_to_reloc = text_section.pointer_to_reloc + (text_section.num_reloc * sizeof(COFF_ImageReloc));
    pdata_section.pointer_to_reloc = data_section.pointer_to_reloc + (data_section.num_reloc * sizeof(COFF_ImageReloc));
    debugs_section.pointer_to_reloc = pdata_section.pointer_to_reloc + (pdata_section.num_reloc * sizeof(COFF_ImageReloc));
    ...
    if (is_xdata_function_info_heap) {
		tb_platform_heap_free(xdata_function_info);
	}
	
    tb_platform_heap_free(xdata_out.data);
    tb_platform_heap_free(debugs_out.data);
    tb_platform_heap_free(debugt_out.data);
