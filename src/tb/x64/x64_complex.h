...
#define VREG_STACK_POINTER \
    (TreeVReg) { 1, VREG_FAMILY_GPR }
#define VREG_BASE_POINTER \
    (TreeVReg) { 2, VREG_FAMILY_GPR }
...
static FunctionTallyComplex tally_memory_usage_complex(TB_Function* restrict f) {
    ...
    return (FunctionTallyComplex) { .memory_usage = tally,
...
static void print_machine_insts(X64_ComplexCtx* restrict ctx) {
    static const char* inst2_names[] = { "ADD", "AND", "OR", "SUB", "XOR", "CMP", "MOV", "TEST",
        "LEA", "IMUL", "XCHG", "MOVSXB", "MOVSXW", "MOVSXD", "MOVZXB", "MOVZXW" };

    static const char* cc_names[] = { "O", "NO", "B", "NB", "E", "NE", "BE", "A", "S", "NS", "P",
        "NP", "L", "GE", "LE", "G" };
    ...
    loop(i, ctx->inst_count) {
        ...
        case INST_JUMP_IF: {
            printf("    J%s L%d ELSE L%d\n", cc_names[inst->jump_if.cond], inst->jump_if.if_true,
                inst->jump_if.if_false);
            break;
        }
        case INST_RET: {
            printf("    RET\n");
        ...
        case INST_COMPARE: {
            printf("    COMPARE gpr:%d, gpr:%d\n", inst->compare.a_vreg.value,
                inst->compare.b_vreg.value);
            break;
        }
        case INST_FOLDED_LOAD: {
            ...
            if (inst->folded_load.index.value) {
                printf("[gpr:%d + gpr:%d * %d", inst->folded_load.base.value,
                    inst->folded_load.index.value, 1 << inst->folded_load.scale);
            } else {
...
static TreeVReg isel(X64_ComplexCtx* restrict ctx, TB_Function* f, TreeNode* tree_node) {
    ...
    case TB_PARAM: {
        ...
        if (ctx->parameters[id].value) {
            ...
            MachineInst inst = { .type = INST_FOLDED_LOAD,
                .dt                    = dt,
                .folded_load           = { .op = MOV,
    ...
    case TB_MUL: {
        ...
        dst = (TreeVReg) { ctx->vgpr_count++, VREG_FAMILY_GPR };
        add_machine_inst(
            ctx, &(MachineInst) { .type = INST_BINARY_OP, .dt = dt, .binary = { op, dst, a, b } });
        break;
    }
    default: tb_todo();
    }
...
static MachineInstMem compute_complex_address(
    ...
    if (reg_type == TB_ARRAY_ACCESS) {
        ...
        return (
            MachineInstMem) { .scale = stride_as_shift, .base = base, .index = index, .disp = 0 };
    } else if (reg_type == TB_MEMBER_ACCESS) {
        return (MachineInstMem) { .scale = SCALE_X1,
...
static void isel_top_level(X64_ComplexCtx* restrict ctx, TB_Function* f, TreeNode* tree_node) {
    ...
    case TB_PHI2: {
        ...
        if (tree_node->operands[0]->use_count == 0 && op_type >= TB_AND && op_type <= TB_MUL) {
            ...
            if (possible_phi->reg == tree_node->reg) {
                ...
                if (rhs->use_count == 0 && rhs_type == TB_LOAD) {
                    ...
                    MachineInst inst = { .type = INST_BINARY_OP_IMM,
                        .dt                    = dt,
                        .binary_imm            = { .op = inst_type,
                            .dst_vreg       = mapping,
                            .a_vreg         = mapping,
                            .b_imm          = imm } };
                    add_machine_inst(ctx, &inst);
                    ...
                    MachineInst inst = { .type = INST_BINARY_OP,
                        .dt                    = dt,
        ...
        add_machine_inst(
            ctx, &(MachineInst) { .type = INST_COPY, .dt = dt, .copy = { mapping, src_vreg } });
        break;
    }
    case TB_STORE: {
        MachineInstMem addr = compute_complex_address(ctx, f, tree_node->operands[0]);
    ...
    case TB_IF: {
        ...
        if (cond_type >= TB_CMP_EQ && cond_type <= TB_CMP_FLE) {
            ...
            add_machine_inst(
                ctx, &(MachineInst) { .type = INST_JUMP_IF, .jump_if = { if_true, if_false, cc } });
        } else {
            tb_todo();
        }
        break;
...
#define idef(r) \
    if (intervals[r].start == 0) intervals[r].start = intervals[r].end = i
...
TB_FunctionOutput x64_complex_compile_function(TB_FunctionID id, TB_Function* restrict f,
    ...
    X64_ComplexCtx* restrict ctx = NULL;
    {
        ...
        if (is_ctx_heap_allocated) {
            ...
            ctx->header.label_patches =
                tb_tls_push(tls, tally.label_patch_count * sizeof(LabelPatch));
    ...
    add_machine_inst(ctx,
        &(MachineInst) {
            .type = INST_EXPLICIT_GPR, .dt = TB_TYPE_PTR, .gpr = { VREG_STACK_POINTER, RSP } });

    add_machine_inst(
        ctx, &(MachineInst) {
                 .type = INST_EXPLICIT_GPR, .dt = TB_TYPE_PTR, .gpr = { VREG_BASE_POINTER, RBP } });
    ...
    loop(i, proto->param_count) {
        ...
        if (dt.width || TB_IS_FLOAT_TYPE(dt.type)) {
            ...
            if (ctx->is_sysv && i < 6) {
                add_machine_inst(ctx, &(MachineInst) { .type = INST_EXPLICIT_GPR,
                                          .dt                = dt,
    ...
    do {
        ...
        if (start->label.id) {
            add_machine_inst(
                ctx, &(MachineInst) { .type = INST_LABEL, .label = { start->label.id } });
