...
#error "Cannot compile JIT for this target architecture!"
    ...
    for (size_t i = 0; i < m->functions.count; i++) {
        ...
        size_t prologue = codegen->get_prologue_length(m->functions.data[i].output->prologue_epilogue_metadata,
													   m->functions.data[i].output->stack_usage);
		
        size_t epilogue = codegen->get_epilogue_length(m->functions.data[i].output->prologue_epilogue_metadata,
													   m->functions.data[i].output->stack_usage);
