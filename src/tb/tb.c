...
TB_API bool tb_module_optimize(TB_Module* m) { return false; }
#undef OPT
...
TB_API void tb_module_destroy(TB_Module* m) {
    ...
    loop(i, m->functions.count) { tb_function_free(&m->functions.data[i]); }
...
TB_API TB_FunctionPrototype* tb_prototype_create(
    ...
    size_t space_needed =
        (sizeof(TB_FunctionPrototype) + (sizeof(uint64_t) - 1)) / sizeof(uint64_t);
    space_needed +=
        ((num_params * sizeof(TB_DataType)) + (sizeof(uint64_t) - 1)) / sizeof(uint64_t);
...
TB_API void tb_initializer_add_global(
    ...
    assert(i->obj_count + 1 <= i->obj_capacity);
    i->objects[i->obj_count++] =
        (TB_InitObj) { .type = TB_INIT_OBJ_RELOC_GLOBAL, .offset = offset, .reloc_global = global };
}
...
void tb_emit_ecall_patch(TB_Module* m, TB_Function* source, TB_ExternalID target_id, size_t pos,
...
static void tb_print_node(
    TB_Function* f, TB_PrintCallback callback, void* user_data, TB_Node* restrict n) {
    ...
    case TB_STRING_CONST:
        ...
        break;
    case TB_LINE_INFO:
        callback(user_data, "  # line %s:%d", f->module->files.data[n->line_info.file].path,
            n->line_info.line);
        break;
    case TB_FLOAT_CONST:
    ...
    case TB_MEMCPY:
        ...
        break;
    case TB_INITIALIZE: callback(user_data, "  initializer r%u, ...", n->init.addr); break;
    case TB_RESTRICT: callback(user_data, "  r%u = restrict", i); break;
    ...
    case TB_ARRAY_ACCESS:
        ...
        callback(user_data, "r%u, r%u, %d", n->array_access.base, n->array_access.index,
            n->array_access.stride);
        break;
    case TB_ATOMIC_XCHG:
    case TB_ATOMIC_ADD:
    case TB_ATOMIC_SUB:
    ...
    case TB_FUNC_ADDRESS: callback(user_data, "  r%u = &%s", i, n->func.value->name); break;
    ...
    case TB_GLOBAL_ADDRESS: {
        TB_GlobalID per_thread_stride = UINT_MAX / TB_MAX_THREADS;
        TB_Global*  g =
            &m->globals[n->global.value / per_thread_stride][n->global.value % per_thread_stride];
    ...
    case TB_STORE:
        ...
        tb_print_type(dt, callback, user_data);
        callback(user_data, " r%u, r%u (%d align)", n->store.address, n->store.value,
            n->store.alignment);
        break;
    case TB_LABEL:
    ...
    case TB_IF:
        callback(
            user_data, "  if (r%u) L%d else L%d", n->if_.cond, n->if_.if_true, n->if_.if_false);
        break;
    case TB_PASS:
...
TB_API void tb_function_print_cfg(TB_Function* f, TB_PrintCallback callback, void* user_data) {
    ...
    do {
        ...
        if (end->type == TB_RET) {
            ...
            callback(
                user_data, "  F%dR%d -> F%dL%d[label=\"true\"]\n", fid, bb, fid, end->if_.if_true);
            callback(user_data, "  F%dR%d -> F%dL%d[label=\"false\"]\n", fid, bb, fid,
                end->if_.if_false);
        } else if (end->type == TB_LABEL) {
            callback(user_data, "  F%dR%d -> F%dL%d\n", fid, bb, fid, end->label.id);
...
void tb_patch2b(TB_Emitter* o, uint32_t pos, uint16_t i) { *((uint16_t*)&o->data[pos]) = i; }

void tb_patch4b(TB_Emitter* o, uint32_t pos, uint32_t i) { *((uint32_t*)&o->data[pos]) = i; }

uint8_t tb_get1b(TB_Emitter* o, uint32_t pos) { return *((uint8_t*)&o->data[pos]); }

uint16_t tb_get2b(TB_Emitter* o, uint32_t pos) { return *((uint16_t*)&o->data[pos]); }

uint32_t tb_get4b(TB_Emitter* o, uint32_t pos) { return *((uint32_t*)&o->data[pos]); }
