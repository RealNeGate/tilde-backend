...
inline static void emit_memory_operand(
    X64_CtxHeader* restrict ctx, uint8_t rx, const Val* restrict a) {
    // Operand encoding
    if (a->type == VAL_GPR || a->type == VAL_XMM) {
        ...
        tb_emit_global_patch(ctx->f->module, ctx->f, (ctx->out - ctx->start_out) - 4, a->global.id,
            s_local_thread_id);
    } else
        tb_unreachable();
}

void inst1(X64_CtxHeader* restrict ctx, Inst1 op, const Val* r) {
    if (r->type == VAL_GPR) {
        ...
        tb_emit_global_patch(ctx->f->module, ctx->f, (ctx->out - ctx->start_out) - 4, r->global.id,
            s_local_thread_id);
    } else
        tb_unreachable();
}
...
void inst2(X64_CtxHeader* restrict ctx, Inst2Type op, const Val* a, const Val* b, int dt_type) {
    ...
    if (inst->ext == EXT_NONE || inst->ext == EXT_DEF || inst->ext == EXT_DEF2) {
        assert(dt_type == TB_I8 || dt_type == TB_I16 || dt_type == TB_I32 || dt_type == TB_I64 ||
               dt_type == TB_PTR);
        ...
        if (a->type == VAL_GPR) {
            ...
            if (base >= 8 || rx >= 8 || is_64bit) { EMIT(rex(is_64bit, rx, base, 0)); }
        } else if (a->type == VAL_MEM) {
            base = a->mem.base;
...
void inst2sse(
    X64_CtxHeader* restrict ctx, Inst2FPType op, const Val* a, const Val* b, uint8_t flags) {
    const static uint8_t OPCODES[] = { [FP_MOV] = 0x10,
        [FP_ADD]                                = 0x58,
        [FP_MUL]                                = 0x59,
        [FP_SUB]                                = 0x5C,
        [FP_DIV]                                = 0x5E,
    ...
    if (supports_mem_dst && dir) { tb_swap(const Val*, a, b); }
    ...
    if (rx >= 8 || base >= 8 || index >= 8) { EMIT(rex(false, rx, base, index)); }
...
inline static void jcc(X64_CtxHeader* restrict ctx, Cond cc, int label) {
    size_t pos = ctx->out - ctx->start_out;
    ctx->label_patches[ctx->label_patch_count++] =
        (LabelPatch) { .pos = pos + 2, .target_lbl = label };
...
inline static void jmp(X64_CtxHeader* restrict ctx, int label) {
    size_t pos = ctx->out - ctx->start_out;
    ctx->label_patches[ctx->label_patch_count++] =
        (LabelPatch) { .pos = pos + 1, .target_lbl = label };
...
// jmp .return
inline static void ret_jmp(X64_CtxHeader* restrict ctx) {
    size_t pos                               = ctx->out - ctx->start_out;
