...
#pragma pack(push,2)
typedef struct X86COFF_ImageReloc {
	union {
...
_Static_assert(sizeof(X86COFF_ImageReloc) == 10, "COFF Image Relocation size != 10 bytes");
...
static void tb_export_coff(TB_Module* m, TB_Arch arch, FILE* f) {
	...
	const char** string_table = (const char**)&tls[m->compiled_functions.count * 4];
	...
	header.symbol_table = text_section.raw_data_pos + text_section.raw_data_size;

	uint32_t string_table_pos = header.symbol_table + (header.symbol_count * sizeof(COFF_Symbol));
