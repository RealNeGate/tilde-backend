...
static TreeNode* walk(TreeNodeArena* arena, TB_Function* f, TB_Reg* use_count, TB_Reg r) {
	...
	switch (n->type) {
		...
		result = push_binary(arena, r, 
							 walk(arena, f, use_count, n->array_access.base),
		...
		case TB_FDIV:
		result = push_binary(arena, r, 
							 walk(arena, f, use_count, n->f_arith.a),
		...
		case TB_SMOD:
		result = push_binary(arena, r, 
							 walk(arena, f, use_count, n->i_arith.a),
		...
		case TB_CMP_FLE:
		result = push_binary(arena, r, 
							 walk(arena, f, use_count, n->cmp.a),
							 walk(arena, f, use_count, n->cmp.b));
		break;
...
TreeNode* tb_tree_generate(TreeNodeArena* arena, TB_Function* f, TB_Reg* use_count, TB_Reg bb, TB_Reg bb_end) {
	...
	if (bb != bb_end) {
		...
		TB_FOR_EACH_NODE_RANGE(n, f, bb, bb_end) {
			...
			switch (reg_type) {
				case TB_STORE: {
					TreeNode* tree_node = push_binary(arena, r, 
													  walk(arena, f, use_count, n->store.address),
													  walk(arena, f, use_count, n->store.value));
...
static void print_node(TreeNode* n, int depth) {
	...
	if (n->operands[0]) {
		print_node(n->operands[0], depth + 1);
	}
	
	if (n->operands[1]) {
		print_node(n->operands[1], depth + 1);
	}
}
